...
class Student
{
    ...
    public bool insertStudent(string fname, string lname, DateTime bdate, string phone, string address, string gender)
    {
        SqlCommand command = new SqlCommand("INSERT INTO Students ([First Name], [Last Name], [Date Of Birth], [Phone], [Address], [Gender]) " +
    ...
    //Create a function to search for a student
    public DataTable searchStudentList(string searchData)
    {
        SqlCommand command = new SqlCommand("SELECT * FROM Students WHERE CONCAT([First Name], [Last Name], [Address]) LIKE '%" + searchData + "%'", connect.getConnection());
        SqlDataAdapter adapter = new SqlDataAdapter(command);
        DataTable dataTable = new DataTable();
        adapter.Fill(dataTable);
        return dataTable;
    ...
    {
        SqlCommand command = new SqlCommand("UPDATE Students SET " +"[First Name] = @FirstName, " + "[Last Name] = @LastName, " +
